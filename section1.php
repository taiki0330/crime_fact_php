...
// }
// SQL作成&実行
$sql = 'SELECT * FROM about_fact WHERE section = 1';
$sqlOrderedAsc = 'SELECT * FROM about_fact WHERE section = 1 ORDER BY crime_date ASC ';
$sqlOrderedDesc = 'SELECT * FROM about_fact WHERE section = 1 ORDER BY crime_date DESC ';


$stmt = $pdo->prepare($sql); //　全体
$stmtOrderedAsc = $pdo->prepare($sqlOrderedAsc); // 昇順
$stmtOrderedDesc = $pdo->prepare($sqlOrderedDesc); // 降順
...
<a href="section1_search.php" style="color: black;">罪名で検索する</a>
<!-- <form action="section1_search.php" method="POST">
  <label for="search_crime_name">罪名で検索</label>
  <input type="text" name="search_crime_name" value="<?php if( !empty($_POST['search_crime_name']) ){ echo $_POST['search_crime_name']; } ?>"></input>
