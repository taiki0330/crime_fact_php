...
// SQL作成&実行
$sql = 'SELECT * FROM about_fact WHERE section = 3';
$sqlOrderedAsc = 'SELECT * FROM about_fact WHERE section = 3 ORDER BY crime_date ASC ';
$sqlOrderedDesc = 'SELECT * FROM about_fact WHERE section = 3 ORDER BY crime_date DESC ';
$sqlSettou = 'SELECT * FROM about_fact WHERE crime_name = "窃盗" ';

$stmt = $pdo->prepare($sql);
$stmtOrderedAsc = $pdo->prepare($sqlOrderedAsc);
$stmtOrderedDesc = $pdo->prepare($sqlOrderedDesc);
$stmtSettou = $pdo->prepare($sqlSettou);
...
try {
    ...
    $statusSettou = $stmtSettou->execute();
...
$resultSettou = $stmtSettou->fetchAll(PDO::FETCH_ASSOC);
// echo '<pre>';
...
foreach($result as $item) {
  ...
  <li class='title_item'>
    ...
    <p class='crime_name'>罪名: <span class='crime_name_3'>{$item["crime_name"]}</span></p>
